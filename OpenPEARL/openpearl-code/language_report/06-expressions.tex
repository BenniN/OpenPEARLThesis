...
%%%\begin{added}
In \OpenPEARL{} the conversions from \code{FLOAT} to \code{FIXED} produce a 
\code{FixedRangeSignal}, if the
data values exeed the value range of the \code{FIXED} type.
The conversion from \code{FIXED} to \code{CHAR} produce a \code{CharacterNotValidSignal},
if the value of the \code{FIXED} expression ist outside the range of 0 to 255.
%%%\end{added}
